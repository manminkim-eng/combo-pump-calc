/* ═══════════════════════════════════════════════════════════════
   ...
   옥내소화전(NFTC 102) + 스프링클러(NFTC 103) 겸용
   ENGINEER KIM MANMIN · Ver 3.0
   캐시 전략:
     · 정적 자산  → Cache-First
...
const APP_ID     = 'manmin-combined-pump';
...
/* ── CDN 호스트 (Stale-While-Revalidate) ── */
const CDN_HOSTS = [
...
self.addEventListener('fetch', e => {
  ...
  if (CDN_HOSTS.some(h => url.hostname.includes(h))) {
    e.respondWith(swrStrategy(req, C_FONTS));
    return;
  }

  /* HTML 탐색 → Network-First */
...
<title>오프라인 — 겸용 펌프 계산서</title>
<style>
*{box-sizing:border-box;margin:0;padding:0;}
body{font-family:'Noto Sans KR',sans-serif;background:#1e3a5f;min-height:100vh;
  display:flex;align-items:center;justify-content:center;padding:24px;}
.card{background:white;border-radius:20px;padding:40px 36px;max-width:440px;
  width:100%;text-align:center;box-shadow:0 24px 64px rgba(0,0,0,0.35);}
.icon{font-size:64px;margin-bottom:16px;line-height:1;}
h1{font-size:20px;font-weight:900;color:#1e3a5f;margin-bottom:8px;}
...
.badge{display:inline-block;border-radius:20px;padding:5px 12px;font-size:11px;font-weight:700;}
.badge-r{background:#fef2f2;color:#b91c1c;border:1px solid #fecaca;}
.badge-g{background:#f0fdf4;color:#15803d;border:1px solid #bbf7d0;}
.badge-b{background:#eff6ff;color:#1d4ed8;border:1px solid #bfdbfe;}
p{font-size:13px;color:#6b7280;line-height:1.8;margin-bottom:20px;}
button{background:#1e3a5f;color:white;border:none;border-radius:10px;
...
<div class="card">
  <div class="icon">📡</div>
  <h1>인터넷 연결 없음</h1>
  <div class="sub">겸용 펌프 용량 계산서</div>
  <div class="badges">
    <span class="badge badge-r">옥내소화전</span>
    <span class="badge badge-g">스프링클러</span>
    <span class="badge badge-b">겸용 펌프</span>
  </div>
